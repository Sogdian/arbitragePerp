...
1. **Bybit** (bybit) готовоОткрЗакр готовоФандинг
2. **Gate.io** (gate) готовоОткрЗакр готовоФандинг
3. **MEXC** (mexc) нельзя
4. **Binance** (binance) готовоОткрЗакр
5. **Bitget** (bitget) готовоОткрЗакр
6. **BingX** (bingx) готовоОткрЗакр
7. **OKX** (okx)
8. **XT.com** (xt)
...
5. **Логика уровней (1→2→…→10):**
   - При открытии лимитных ордеров анализируется стакан **до 10 уровней**
...
### Автоматическое закрытие позиций (position_opener.py)

**Описание:**
Модуль `position_opener.py` реализует автоматическое закрытие Long и Short позиций на биржах при достижении триггера: **3 Telegram-сообщения про "закрытие при спреде" в течение 1 минуты**.

**Триггер закрытия:**
- Отслеживаются **только** сообщения типа "📱 Отправлено сообщение в Telegram: закрытие при спреде X% <= Y%"
- Сообщения про `fr_spread` **не учитываются** в подсчете
- **Окно времени:** фиксированное **1 минута (60 секунд)**
- Если за это окно отправилось **3 таких сообщения** → автоматически закрываются позиции на обеих биржах
...
- Режим исполнения: **IOC (Immediate-Or-Cancel)** — допускается частичное исполнение
- **Не используется FOK** — можно закрывать несколькими ордерами
- **`reduceOnly=True`** — критично для закрытия, особенно в hedge/dual режиме (гарантирует, что ордер только уменьшает позицию, а не открывает/увеличивает)
- Глубина стакана: **максимум 10 уровней** для подбора цены
- Логика: пробуем закрыть на уровне 1 → если не исполнилось, пробуем уровень 2 → ... → уровень 10
- Можно отправлять **несколько ордеров** подряд, пока весь объем не закроется

**Процесс закрытия позиций:**

1. **Триггер в мониторинге:**
   - В `monitor_spreads()` отслеживаются временные метки отправки Telegram-сообщений про "закрытие при спреде"
   - При каждой отправке добавляется метка времени в список `close_alert_times`
   - Старые метки (старше 60 секунд) удаляются
   - Если в списке накопилось **3 метки** → вызывается `close_long_short_positions()`

2. **Закрытие Long позиции:**
   - На бирже `long_exchange`: выставляются лимитные **Sell** ордера (закрываем Long = продаем)
   - Цена берется из стакана: `bids` (лучшая цена покупки)
   - Используется `reduceOnly=True` для гарантии закрытия, а не открытия новой позиции

3. **Закрытие Short позиции:**
   - На бирже `short_exchange`: выставляются лимитные **Buy** ордера (закрываем Short = покупаем)
   - Цена берется из стакана: `asks` (лучшая цена продажи)
   - Используется `reduce_only=True` для гарантии закрытия

4. **Частичное исполнение:**
   - Если ордер исполнился частично → остаток закрывается следующим ордером
   - Отслеживается средняя цена исполнения (VWAP) для каждой ноги
   - Процесс продолжается до полного закрытия или исчерпания попыток

5. **Проверка успешности:**
   - После закрытия проверяется, что обе ноги закрыты полностью
   - Если обе ноги закрыты → мониторинг **останавливается**
   - Если хотя бы одна нога не закрыта → мониторинг **продолжается** (счетчик алертов сбрасывается)

**Логирование:**
- При успешном закрытии выводится:
  ```
  ✅ Позиции закрыты: DASH | Long bybit + Short gate | 
  Цена выхода Long: 85.57, Цена выхода Short: 85.66, 
  Спред закрытия: 0.105%, Количество монет: 0.07
  ```
- Все числовые значения форматируются до 3 знаков после запятой с удалением нулей на конце
- Выводится информация о каждом ордере закрытия, количестве исполненных контрактов/монет, остатке

**Расчет спреда закрытия:**
- Формула: `(bid_long - ask_short) / ask_short * 100`
- Где `bid_long` — средняя цена закрытия Long (продажа), `ask_short` — средняя цена закрытия Short (покупка)
- Используется VWAP (средневзвешенная цена) для каждой ноги
...
- Отслеживание исполнения через `/v5/order/realtime` и `/v5/order/history`

**Gate.io:**
- Используется `reduce_only=True` в теле ордера (критично для hedge/dual режима)
...
- После успешного закрытия мониторинг останавливается
- Если закрытие не удалось → мониторинг продолжается (счетчик алертов сбрасывается)
