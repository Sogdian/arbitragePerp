...
5. **news_monitor.py** - Модуль для мониторинга новостей о делистинге монет
...
6. **announcements_monitor.py** - Модуль для мониторинга новостей о безопасности/взломах
...
7. **config.py** - Конфигурация (логирование, Telegram настройки)

8. **telegram_sender.py** - Модуль для отправки сообщений в Telegram каналы
...
```
Закр (min): -4.27% (0.18%) | Откр (max): 3.92% | long_fr: -0.25 | short_fr: 0.01 | fr_spread: 0.263 (откр: ≥0.18%, закр: ≤0.05%)
```
...
- `fr_spread: 0.263 (откр: ≥0.18%, закр: ≤0.05%)` - за один funding-период будет прибыль 0.263% от фандинга. Порог открытия (≥0.18%) достигнут, можно открывать позиции. Порог закрытия (≤0.05%) не достигнут, закрывать еще рано.
...
async def fetch(bot, ex, coin):
    async with sem:
        return await bot.get_futures_data(ex, coin, need_funding=False)
```
- Предотвращает перегрузку API rate-limit
- Настраивается через `SCAN_MAX_CONCURRENCY` (по умолчанию 40)
- Используется `need_funding=False` для пропуска запросов funding rate, что снижает количество API-запросов в 2 раза и убирает "шум" в логах от ошибок получения funding rate
...
# Таймаут на запрос к бирже в секундах
...
# Размер батча монет для обработки (в данный момент не используется, но может быть полезен в будущем)
...
- При проблемах с таймаутами → увеличить `SCAN_REQ_TIMEOUT_SEC` до 15-20 секунд
- При большом количестве монет (800+) можно увеличить `SCAN_COIN_BATCH_SIZE` до 100 (если батчинг будет использоваться)
...
5. После каждого батча выводится прогресс: `Progress: {processed}/{total} coins processed`
6. Найденные возможности сортируются по спреду (desc)
7. Выводятся только возможности с `spread >= MIN_SPREAD` с префиксом `💰` (эмодзи)
8. После завершения цикла выводится время выполнения: `scan_once finished in {time:.1f}s; sleeping {interval:.1f}s`
...
**Примечание:** Префикс `💰` (эмодзи) используется для визуального выделения найденных арбитражных возможностей в логах.
...
📈 (Long bingx) (CLO) Price: 0.7396
📈 (Long bingx) (CLO) Funding: -0.116700%
📉 (Short gate) (CLO) Price: 0.75479
📉 (Short gate) (CLO) Funding: -0.167300%

📊 Price spread: 2.0538%
📊 Funding spread: -0.050600% (open: ≥0.18%, close: ≤0.05%)

✅ ✓ Liquidity bingx Long (CLO): 30.0 USDT | spread=2.7bps, buy_impact=0.0bps
✅ ✓ Liquidity gate Short (CLO): 30.0 USDT | spread=9.0bps, sell_impact=0.4bps

✅ ✓ No delisting news for CLO (bingx, gate) in the last 60 days
✅ ✓ No security/hack news for CLO (bingx, gate) in the last 60 days

CLO Long (bingx), Short (gate)
```

**Важно:**
- Все данные для сообщения берутся из сканера, без дополнительных API-запросов
- Если фандинг не запрашивался в сканере, он не будет показан в сообщении
