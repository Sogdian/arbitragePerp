...
5. Для каждой найденной связки со спредом >= MIN_SPREAD:
   ...
   - Выводится одна строка: `💰 {coin} Long ({long_ex}), Short ({short_ex}) spread {spread:.4f}% {verdict}`
...
```
💰 BTC Long (gate), Short (bingx) spread 2.3456% ✓ арбитражить
💰 ETH Long (bybit), Short (okx) spread 2.1234% ✗ не арбитражить
💰 SOL Long (mexc), Short (bitget) spread 2.5678% ✓ арбитражить
```
