...
async def fetch(bot, ex, coin):
    async with sem:
        # Тикер и funding запрашиваются отдельно
...
- Настраиваемые таймауты: `SCAN_TICKER_TIMEOUT_SEC`, `SCAN_FUNDING_TIMEOUT_SEC`
- Поддержка ретраев для тикера: `SCAN_FETCH_RETRIES` (по умолчанию 1)
...
# Таймаут на запрос к бирже в секундах (общий)
...
**Особенности формата:**
- HTML-разметка для форматирования (жирный текст, моноширинный шрифт для цен/фандингов)
- Разделители из символов ━ для визуального разделения секций
- Древовидная структура (├ └) для отображения данных LONG/SHORT
...
- Если фандинг отсутствует (timeout или ошибка), показывается "N/A"
