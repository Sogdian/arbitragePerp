...
**Важно:** Размер инвестиций в USDT обязателен. Бот использует этот размер для проверки ликвидности на обеих биржах (Long и Short).
...
```
🚩 Закр: -4.27% (min: 0.18%) | ⛳ Откр: 3.92% | 📊 pr_spread: 3.9200 | 💰 fr_spread: 0.263 (≥0.18%, ≤0.05%) ⚙️ Long binance | Short bybit | ZKP
```
...
- **💰 fr_spread:** Чистый эффект по фандингу, рассчитывается по формуле `funding_short - funding_long` (в процентах)
  ...
  - **Пороги для открытия/закрытия:**
    - Открываем позиции, когда `fr_spread ≥ 0.18%` (покрывает комиссии и запас)
    - Закрываем позиции, когда `fr_spread ≤ 0.05%` (минимальный порог для закрытия)
...
- `💰 fr_spread: 0.263 (≥0.18%, ≤0.05%)` - за один funding-период будет прибыль 0.263% от фандинга. Порог открытия (≥0.18%) достигнут, можно открывать позиции. Порог закрытия (≤0.05%) не достигнут, закрывать еще рано.
...
📈 Long gate Ц/Ф: 1.232 / -1.170% | 📉 Short mexc Ц/Ф: 1.25 / 0.013% | 📊 Спред Ц/Ф: 1.055% / 1.183% | ✅ арбитражить
📈 Long gate Ц/Ф: 1.232 / -1.170% | 📉 Short bingx Ц/Ф: 1.231 / 0.009% | 📊 Спред Ц/Ф: -0.073% / 1.179% | ❌ не арбитр (спред -0.073% < 2.0%)
📈 Long mexc Ц/Ф: 1.25 / 0.013% | 📉 Short gate Ц/Ф: 1.232 / -1.170% | 📊 Спред Ц/Ф: -1.440% / -1.183% | ❌ не арбитр (ликв. Long: ask1: 1.25 bid1: 1.248 spread 48.4 bps > 30.0)
📈 Long mexc Ц/Ф: 1.25 / 0.013% | 📉 Short bingx Ц/Ф: 1.231 / 0.009% | 📊 Спред Ц/Ф: -1.520% / -0.004% | ❌ не арбитр (спред -1.520% < 2.0%; делистинг)
```
...
- **📊 Спред Ц/Ф:** Спред на цену и спред на фандинги
- **Вердикт:** `✅ арбитражить` или `❌ не арбитр`
...
```
💰 BTC Long (gate), Short (bingx) spread 2.346% | fund 0.003700 ✅ арбитражить
💰 ETH Long (bybit), Short (okx) spread 2.123% | fund -0.022498 ❌ не арбитражить (ликв. Long: ask1: 1.23 bid1: 1.22 spread 48.4 bps > 30.0)
💰 SOL Long (mexc), Short (bitget) spread 2.568% | fund 0.001200 ✅ арбитражить
💰 ALL Long (gate), Short (binance) spread 10.121% | fund -0.022 ❌ не арбитражить (ликв. Short: sell impact 65.2 bps > 50.0; делистинг)
```
...
- `fund` - сокращение от `funding`, показывает спред на фандинги (funding spread)
...
### Режимы работы
- **Тестовый режим** (по умолчанию): `ENV=test` или без указания переменной
  - Сообщения отправляются в `TEST_CHANNEL_ID`
- **Продакшн режим**: `ENV=prod`
  - Сообщения отправляются в `FREE_CHANNEL_ID`
...
- Канал настроен для текущего режима (`TEST_CHANNEL_ID` для test, `FREE_CHANNEL_ID` для prod)
...
📊 Spreads:
• Price Spread: 2.6116%
• Funding Spread: -0.110950%

⚙️ Settings: Open ≥ 0.18% | Close ≤ 0.05%
...
### Исправление проблемы с монетой FUN на MEXC
- **Проблема:** На MEXC под монетой `FUN` бот получал данные от неправильного контракта:
  - UI отображает `FUNUSDT` с ценой ~0.07 (это контракт `SPORTFUN_USDT`)
  - Но бот запрашивал `FUN_USDT`, который является контрактом `FUNTOKEN_USDT` с ценой ~0.002
  - Это приводило к "космическим" спредам (3199.383%) и неправильному расчету количества монет (20576.132 FUN вместо ~700 FUN)
- **Решение:**
  - Добавлен маппинг алиасов в `exchanges/async_mexc.py` в методе `_normalize_symbol()`:
    - `FUN` → `SPORTFUN_USDT` (соответствует UI FUNUSDT)
    - `FUNTOKEN` → `FUN_USDT` (соответствует UI FUNTOKENUSDT)
  - Обновлен `exchanges/coin_list_fetchers.py` для MEXC: теперь используется `displayName` из API для определения имени монеты, чтобы соответствовать UI
- **Результат:** Теперь бот корректно получает данные по `FUN` с ценой ~0.07, спреды и количество монет рассчитываются правильно
...
### Исправление MEXC алиаса для FUN (и некорректных “космических” спредов)
- **Симптом:** для `FUN Long (mexc), Short (gate)` появлялся спред ~`3199%` и “количество монет” считалось некорректно (из-за неправильной цены на MEXC).
- **Причина:** на MEXC есть два разных контракта:
  - `FUN_USDT` = `FUNTOKEN_USDT` (цена ~0.002)
  - `SPORTFUN_USDT` = UI `FUNUSDT` (цена ~0.07)
  Раньше `coin=FUN` на MEXC всегда маппился в `FUN_USDT`, что не совпадало с UI и с другими биржами.
- **Решение:**
  - В `exchanges/async_mexc.py` добавлен алиас `FUN -> SPORTFUN_USDT` (и `FUNTOKEN -> FUN_USDT`)
  - В `exchanges/coin_list_fetchers.py` для MEXC имя монеты теперь извлекается из `displayName`, если он доступен (чтобы `FUN` соответствовал UI).
