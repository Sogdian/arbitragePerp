...
- **Проверка времени:** Если скрипт запущен после времени выплаты (например, в 12:05, а время выплаты было 12:00) → завершать с ошибкой
...
- Плечо: **1x**
- Исполнение: **Только полностью** (FOK) или отмена
...
- **Точность:** Фиксация цены в 11:59:59, открытие ордеров в 12:00:00
...
- **Short:** Лимитный ордер по лучшей цене **bid** из стакана (FOK)
- **Long:** Лимитный ордер на рассчитанном уровне (FOK)
- **Исполнение:** Только полностью или отмена (FOK)
- **Маржа:** Изолированная (isolated)
- **Плечо:** 1x
...
1. **position_opener.py:**
   ...
   - Адаптировать под требования FOK и одновременного открытия
...
3. **Проверки перед исполнением:**
   - Баланс достаточен для обеих позиций
   - Минимальные/максимальные размеры соблюдены
   - Шаги цены и количества корректны
   - Ликвидность на нужных уровнях стакана: проверка **сразу на всех 15 уровнях** для Long
...
1. **Парсер команды:** Разбор формата `"STO Bybit 30 -2% 12:00 -0.3%"`
   - Регистр не важен, пробелы строго один между параметрами
   - Формат процентов: только с знаком `%` (например, `-2%`, `-0.3%`)
   - Проценты только отрицательные
   - Проверка биржи: наличие в списке поддерживаемых
...
3. **FOK ордера:** Только полное исполнение или отмена
