...
### Цель
Непрерывное сканирование монет на Bybit для поиска фандингов >= `MIN_FUNDING` с отправкой уведомлений в Telegram.
...
if MIN_FUNDING < 0:
    ...
    # Для положительных порогов: ищем >= MIN_FUNDING
    if funding_rate_pct < MIN_FUNDING:
        return None
...
- `MIN_FUNDING=-1` — порог фандинга (в %, например -1 = ищем <= -1%)
...
**Лог:**
```
scan_fundings started | MIN_FUNDING=-1.00% | interval=60s | telegram=enabled
