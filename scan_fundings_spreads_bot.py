...
async def main():
    ...
    try:
        ...
        print("\nОткрыть позиции в лонг и шорт? Введите 'Да' или 'Нет': если 'Да', то позиции будут открыты.")
        print("Опционально можно указать min цену (через .) закр для отправки сообщения в тг и закрытия всех позиций (например: 'Да, 1' или 'Да, 0.5').")
        sys.stdout.flush()
